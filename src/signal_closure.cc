...
static void gir_signal_closure_marshal(GClosure *closure,
    ...
    for (guint i = 0; i < n_param_values; i++) {
        ...
        GIArgInfo *arg_info =
            g_callable_info_get_arg(gir_signal_closure->signal_info,
                                    i); // FIXME: CRITICAL: we should assert that the length of the callable params
                                        // matches the n_param_values to avoid an array overrun!!!!!!!
        GITypeInfo *type_info = g_arg_info_get_type(arg_info);

        // convert the native GValue to a v8::Value
        Local<Value> js_param =
            GIRValue::from_g_value(&native_param, type_info); // TODO: this can throw nan errors if the
    ...
    Nan::MaybeLocal<Value> maybe_result =
        Nan::Call(local_callback, Nan::GetCurrentContext()->Global(), n_param_values, callback_argv.data());
