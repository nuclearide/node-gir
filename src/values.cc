  ...
  switch (G_TYPE_FUNDAMENTAL(type)) {
    ...
    case G_TYPE_BOXED:
      if (G_VALUE_TYPE(v) == G_TYPE_ARRAY) {
        ...
        boxed_info =
            g_irepository_find_by_gtype(NamespaceLoader::repo, G_VALUE_TYPE(v));
        return GIRStruct::from_existing((GIRStruct *)g_value_get_boxed(v),
                                        boxed_info);
      }
      break;
...
bool GIRValue::to_g_value(Handle<Value> value, GType type, GValue *v) {
  ...
  switch (G_TYPE_FUNDAMENTAL(type)) {
    ...
    case G_TYPE_OBJECT:
      if (value->IsObject()) {
        g_value_set_object(
            v, Nan::ObjectWrap::Unwrap<GIRObject>(value->ToObject())->obj);
        return true;
      }
      break;
    ...
    case G_TYPE_BOXED:
      g_value_set_boxed(
          v,
          Nan::ObjectWrap::Unwrap<GIRStruct>(value->ToObject())->c_structure);
      return true;
