...
GIArgument Args::get_out_argument_value(GIArgInfo &argument_info) {
  ...
  if (g_arg_info_is_caller_allocates(&argument_info)) {
    ...
    if (arg_type_tag == GI_TYPE_TAG_INTERFACE) {
      GIRInfoUniquePtr argument_interface_info =
          GIRInfoUniquePtr(g_type_info_get_interface(&argument_type_info));
      GIInfoType argument_interface_type =
          g_base_info_get_type(argument_interface_info.get());
      gsize argument_size;

      if (argument_interface_type == GI_INFO_TYPE_STRUCT) {
        argument_size = g_struct_info_get_size(
            (GIStructInfo *)argument_interface_info.get());
      } else if (argument_interface_type == GI_INFO_TYPE_UNION) {
        argument_size =
            g_union_info_get_size((GIUnionInfo *)argument_interface_info.get());
      } else {
        stringstream message;
        message << "type \"" << g_type_tag_to_string(arg_type_tag)
                << "\" for out caller-allocates";
...
GIArgument Args::to_g_type(GIArgInfo &argument_info, Local<Value> js_value) {
  ...
  if (js_value->IsNullOrUndefined() &&
      (g_arg_info_may_be_null(&argument_info) ||
       argument_type_tag == GI_TYPE_TAG_VOID)) {
    ...
    switch (argument_type_tag) {
      ...
      case GI_TYPE_TAG_INTERFACE: {
        ...
        if (g_type_is_a(gtype, G_TYPE_OBJECT)) {
          ...
          GIRObject *gir_object =
              Nan::ObjectWrap::Unwrap<GIRObject>(js_value->ToObject());
        ...
        if (g_type_is_a(gtype, G_TYPE_VALUE)) {
          GValue gvalue = {0, {{0}}};
          if (!GIRValue::to_g_value(js_value, G_TYPE_INVALID, &gvalue)) {
...
Handle<Value> Args::from_g_type_array(GIArgument *arg, GITypeInfo *type,
                                      int array_length) {
  ...
  switch (param_tag) {
    case GI_TYPE_TAG_UINT8:
      if (arg->v_pointer == nullptr) return Nan::New("", 0).ToLocalChecked();
    ...
    case GI_TYPE_TAG_GTYPE:
      if (arg->v_pointer == nullptr) return Nan::New<Array>();
      arr = Nan::New<Array>(array_length);
      for (i = 0; i < array_length; i++) {
        Nan::Set(
            arr, i,
            Nan::New((int)GPOINTER_TO_INT(((gpointer *)arg->v_pointer)[i])));
      }
      return arr;
    ...
    case GI_TYPE_TAG_INTERFACE:
      ...
      for (i = 0; i < array_length; i++) {
        ...
        arr->Set(i, GIRObject::from_existing(
                        o, g_registered_type_info_get_g_type(interface_info)));
      }
    ...
    default:
      gchar *exc_msg =
          g_strdup_printf("Converting array of '%s' is not supported",
...
Local<Value> Args::from_g_type(GIArgument *arg, GITypeInfo *type,
                               int array_length) {
  ...
  if (tag == GI_TYPE_TAG_INTERFACE) {
    ...
    if (interface_type == GI_INFO_TYPE_OBJECT) {
      Local<Value> new_instance = GIRObject::from_existing(
  ...
  if (tag == GI_TYPE_TAG_INTERFACE) {
    ...
    if (g_type_is_a(gtype, G_TYPE_OBJECT)) {
      ...
      return GIRObject::from_existing(o, G_OBJECT_TYPE(o));
    }
    if (g_type_is_a(gtype, G_TYPE_VALUE)) {
      GIRValue::from_g_value((GValue *)arg->v_pointer, nullptr);
    }
  }
