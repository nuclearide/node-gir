...
Handle<Value> GIRStruct::from_existing(gpointer c_structure,
                                       GIStructInfo *info) {
  ...
  for (it = templates.begin(); it != templates.end(); ++it) {
    if (g_base_info_equal(info, it->info)) {
      Local<Function> instance_constructor =
          Nan::GetFunction(Nan::New(it->function)).ToLocalChecked();
  ...
  if (res == Nan::Null()) {
    ...
    g_warning(
        "GIRStruct does not currently support creating new instances from "
  ...
  if (!res.IsEmpty()) {
    ...
    s->info = info;
    if (s->c_structure) g_free(s->c_structure);
...
Local<Value> GIRStruct::prepare(GIStructInfo *info) {
  ...
  if (g_struct_info_is_gtype_struct(info)) return Nan::Null();
  ...
  Local<External> struct_info_extern =
      Nan::New<External>((void *)g_base_info_ref(info));
  Local<FunctionTemplate> object_template =
      Nan::New<FunctionTemplate>(GIRStruct::New, struct_info_extern);
  ...
  v8::Handle<v8::External> info_handle =
      Nan::New<v8::External>((void *)g_base_info_ref(info));
  // Set fields handlers
  SetNamedPropertyHandler(object_instance_template, FieldGetHandler,
                          FieldSetHandler, FieldQueryHandler, 0, 0,
                          info_handle);
...
void GIRStruct::initialize(Handle<Object> target, char *namespace_) {
  ...
  for (it = templates.begin(); it != templates.end(); ++it) {
    if (strcmp(it->namespace_, namespace_) == 0) {
      target->Set(
          Nan::New<String>(g_base_info_get_name(it->info)).ToLocalChecked(),
...
Handle<Value> GIRStruct::get_structure(gpointer c_structure) {
  ...
  for (it = instances.begin(); it != instances.end(); it++) {
    if (it->gir_structure && it->gir_structure->c_structure &&
        it->gir_structure->c_structure == c_structure) {
...
NAN_METHOD(GIRStruct::CallMethod) {
    ...
    GIRStruct *that = Nan::ObjectWrap::Unwrap<GIRStruct>(info.This()->ToObject());
    Local<Value> result =
        Func::call((GObject *)that->c_structure, function_info, info);
...
Handle<Object> GIRStruct::property_list(GIObjectInfo *info) {
  ...
  while (true) {
    ...
    for (int i = 0; i < l; i++) {
      ...
      list->Set(Nan::New<Number>(i + gcounter),
...
Handle<Object> GIRStruct::method_list(GIObjectInfo *info) {
  ...
  while (true) {
    ...
    for (int i = 0; i < l; i++) {
      ...
      list->Set(Nan::New<Number>(i + gcounter),
                Nan::New(g_base_info_get_name(func)).ToLocalChecked());
...
void GIRStruct::register_methods(GIStructInfo *info, const char *namespace_,
  ...
  for (int i = 0; i < number_of_methods; i++) {
    ...
    Local<String> function_name =
        Nan::New(js_func_name.c_str()).ToLocalChecked();
    ...
    if ((func_flag & GI_FUNCTION_IS_CONSTRUCTOR)) {
      ...
      Local<External> function_info_extern =
          Nan::New<External>((void *)g_base_info_ref(func));
      Local<FunctionTemplate> method_template = Nan::New<FunctionTemplate>(
          GIRStruct::CallMethod, function_info_extern);
