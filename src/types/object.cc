...
Local<Value> GIRObject::from_existing(GObject *existing_gobject,
                                      GType gobject_type) {
  ...
  MaybeLocal<Value> existing_gir_object =
      GIRObject::get_instance(existing_gobject);
  if (!existing_gir_object.IsEmpty()) {
    return existing_gir_object.ToLocalChecked();
  }

  GIObjectInfo *object_info = (GIObjectInfo *)g_irepository_find_by_gtype(
      NamespaceLoader::repo, gobject_type);
  ...
  ObjectFunctionTemplate *oft =
      GIRObject::find_or_create_template_from_object_info(object_info);
  Local<Function> instance_constructor =
      Nan::GetFunction(Nan::New(oft->object_template)).ToLocalChecked();
  Local<Object> instance =
      Nan::NewInstance(instance_constructor).ToLocalChecked();
...
NAN_METHOD(GIRObject::New) {
    ...
    if (info.Length() == 1 && info[0]->IsObject()) {
      properties = GIRObject::parse_constructor_argument(info[0]->ToObject(),
                                                         object_info);
    }
...
GType GIRObject::get_object_property_type(GIObjectInfo *object_info,
                                          const char *property_name) {
  void *klass =
      g_type_class_ref(g_registered_type_info_get_g_type(object_info));
  GParamSpec *param_spec =
      g_object_class_find_property(G_OBJECT_CLASS(klass), property_name);
...
map<string, GValue> GIRObject::parse_constructor_argument(
  ...
  for (size_t i = 0; i < property_names->Length(); i++) {
    ...
    GType property_g_type =
        GIRObject::get_object_property_type(object_info, *property_name_cstr);

    GValue gvalue = {0, {{0}}};
    if (!GIRValue::to_g_value(properties_object->Get(property_name),
                              property_g_type, &gvalue)) {
      gchar *msg = g_strdup_printf("'%s' property value conversion failed",
                                   *property_name_cstr);
    ...
    properties.insert(
        pair<string, GValue>(string(*property_name_cstr), gvalue));
  }
...
NAN_PROPERTY_SETTER(PropertySetHandler)
{
    ...
    if (base_info != nullptr) {
        ...
        if (pspec) {
            ...
            GValue gvalue = {0, {{0}}};
            bool value_is_set =
                GIRValue::to_g_value(value, pspec->value_type, &gvalue);
...
ObjectFunctionTemplate *GIRObject::create_object_template(
    GIObjectInfo *object_info) {
  Local<External> object_info_extern =
      Nan::New<External>((void *)g_base_info_ref(object_info));
  Local<FunctionTemplate> object_template =
      Nan::New<FunctionTemplate>(GIRObject::New, object_info_extern);

  ObjectFunctionTemplate *oft =
      new ObjectFunctionTemplate();  // TODO: where do we deallocate? When the
                                     // namespace object (the node module) is
                                     // collected?
  g_base_info_ref(
      object_info);  // ref the info because we're storing an reference on 'oft'
  oft->info = object_info;
  oft->object_template = PersistentFunctionTemplate(
      object_template);  // TODO: refactor oft->object_template to
  ...
  v8::Local<v8::ObjectTemplate> object_instance_template =
      object_template->InstanceTemplate();
  ...
  v8::Handle<v8::External> info_handle =
      Nan::New<v8::External>((void *)g_base_info_ref(oft->info));
  // Set properties handlers
  SetNamedPropertyHandler(object_instance_template, PropertyGetHandler,
                          PropertySetHandler, PropertyQueryHandler, nullptr,
                          nullptr, info_handle);
  ...
  for (int i = 0; i < number_of_constants; i++) {
    ...
    object_template->Set(
        Nan::New(g_base_info_get_name(constant)).ToLocalChecked(),
...
Local<Object> GIRObject::prepare(GIObjectInfo *object_info) {
  ObjectFunctionTemplate *oft =
      find_or_create_template_from_object_info(object_info);
...
void GIRObject::extend_parent(Local<FunctionTemplate> &object_template,
                              GIObjectInfo *object_info) {
  ...
  ObjectFunctionTemplate *oft =
      GIRObject::find_or_create_template_from_object_info(parent_object_info);
...
ObjectFunctionTemplate *GIRObject::find_template_from_object_info(
    GIObjectInfo *object_info) {
...
ObjectFunctionTemplate *GIRObject::find_or_create_template_from_object_info(
    GIObjectInfo *object_info) {
  ObjectFunctionTemplate *oft =
      GIRObject::find_template_from_object_info(object_info);
  if (oft == nullptr) {
...
void GIRObject::set_custom_fields(Local<FunctionTemplate> &object_template,
                                  GIObjectInfo *object_info) {
  ...
  object_template->Set(Nan::New("__properties__").ToLocalChecked(),
                       GIRObject::property_list(object_info));
  object_template->Set(Nan::New("__methods__").ToLocalChecked(),
                       GIRObject::method_list(object_info));
  object_template->Set(Nan::New("__interfaces__").ToLocalChecked(),
                       GIRObject::interface_list(object_info));
  object_template->Set(Nan::New("__fields__").ToLocalChecked(),
                       GIRObject::field_list(object_info));
  object_template->Set(Nan::New("__signals__").ToLocalChecked(),
                       GIRObject::signal_list(object_info));
  object_template->Set(Nan::New("__v_funcs__").ToLocalChecked(),
                       GIRObject::v_func_list(object_info));
  object_template->Set(
      Nan::New("__abstract__").ToLocalChecked(),
      Nan::New<Boolean>(g_object_info_get_abstract(object_info)));
}

void GIRObject::set_custom_prototype_methods(
    Local<FunctionTemplate> &object_template) {
  Nan::SetPrototypeMethod(object_template, "__get_property__",
                          GIRObject::GetProperty);
  Nan::SetPrototypeMethod(object_template, "__set_property__",
                          GIRObject::SetProperty);
  Nan::SetPrototypeMethod(object_template, "__get_interface__",
                          GIRObject::GetInterface);
  Nan::SetPrototypeMethod(object_template, "__get_field__",
                          GIRObject::GetField);
...
GISignalInfo *GIRObject::find_signal(GIObjectInfo *inf, char *name) {
  ...
  if (!signal) {
    ...
    if (!signal) signal = _object_info_find_interface_signal(inf, name);
  }
  return signal;
}
...
Local<v8::ObjectTemplate> GIRObject::property_list(GIObjectInfo *info) {
  ...
  while (true) {
    if (!first) {
      ...
      if (strcmp(g_base_info_get_name(parent), g_base_info_get_name(info)) ==
          0) {
        return list;
      }
 ...
 */
void GIRObject::set_method(Local<FunctionTemplate> &target,
  ...
  if (g_function_info_get_flags(function_info) & GI_FUNCTION_IS_METHOD) {
    ...
    target->PrototypeTemplate()->Set(js_function_name,
                                     Func::create_method(function_info));
  } else {
...
Handle<ObjectTemplate> GIRObject::interface_list(GIObjectInfo *info) {
  ...
  while (true) {
    ...
    for (int i = 0; i < l; i++) {
      GIInterfaceInfo *interface = g_object_info_get_interface(info, i);
      list->Set(
          Nan::New(std::to_string(i + gcounter)).ToLocalChecked(),
...
Handle<ObjectTemplate> GIRObject::signal_list(GIObjectInfo *info) {
  ...
  while (true) {
    if (!first) {
      ...
      if (strcmp(g_base_info_get_name(parent),
                 g_base_info_get_name(info) /*"InitiallyUnowned"*/) == 0) {
        return list;
      }
      g_base_info_unref(info);
      info = parent;
